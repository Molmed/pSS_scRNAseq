...
p2 <- plot_grid(plotlist = p, ncol = 3)

ggsave2(paste0("../results/Fig1C.png"), 
        plot_grid(p1, p2, ncol = 2, rel_widths = c(1.5, 1)) + bgcolor("white"), 
...
p <- DimPlot(gex, 
             reduction = "umap_2", 
             group.by = "cluster_cellType_manual_fine",
             label = TRUE,label.size = 4, label.box = TRUE) + 
  xlab("UMAP 1") + 
  ylab("UMAP 2") + 
  ggtitle("celltypes") + 
    theme(axis.text = element_blank(),
...
#  patient subtypes per cell type Fig1F
...
p_legend <- ggplot(gex@meta.data, aes(x = reorder(cluster_cellType_manual_fine,
        ...
        theme_classic() +
        geom_bar(position = "fill") + 
        scale_fill_brewer(palette = "Set3", na.value = "grey", name = "Patient group")

p_legend <- get_legend(p_legend) %>% ggpubr::as_ggplot()

p3 <- plot_grid(p1, p2, ncol = 1, labels = NULL, align = "v")

ggsave2("../results/cell_types_fine_perPatientGroup_2.png", 
        plot_grid(p3, 
                  p_legend, 
                  ncol = 2, 
                  rel_widths = c(5, 1), 
...
p3 <- plot_grid(p1, p2, p4, ncol = 1, labels = NULL, rel_heights = c(1, 1, 3) , align = "v")

ggsave2("../results/cell_types_fine_perPatientGroup_3.png", 
        plot_grid(p3, 
                  p_legend, 
                  ncol = 2, 
                  rel_widths = c(5, 1), 
...
# barplot with cell types per patient
# p2 <- ggplot(gex@meta.data, aes(y = patient_group, 
...
#  patient subtypes per cell type Fig1F, split plot move Naive cells to the side using a different scale
# and add statistics

```{r}

gex.meta <- gex@meta.data
table(gex$orig.ident)
table(gex$orig.ident, gex$patient_group)
p1.1 <- ggplot(gex.meta[gex.meta$cluster_cellType_manual_fine == "Naive"], aes(x = reorder(cluster_cellType_manual_fine,
                                        -table(cluster_cellType_manual_fine)[cluster_cellType_manual_fine]), 
                                            fill = patient_group)) +
        theme_classic() +
        geom_bar(position = position_stack(reverse = FALSE)) + 
