...
p_list <- list()
for (i in sort(unique(gex$cluster_cellType_manual_fine))[-c(16)]){
...
ggsave2("../results/DGE_B-cells_celltypeFine_dotplots_3.png", 
        plot_grid(plotlist = p_list, ncol = 3, labels = NULL, align = "hv"), 
        width = 26, height = 32, unit = "cm")


```


### GSEA celltypesFine (GO_Biological_Process_2021 and MSigDB_Hallmark_2020)
...
ggsave2(paste0("../results/DGE_B-cells_GSE_celltypeFine_hallmarks.png"),
...
ggsave2(paste0("../results/DGE_B-cells_GSE_celltypeFine_GOBP.png"),
          plot_grid(plotlist = p_listBP, 
                    ncol = 3, 
                    labels = NULL, 
                    align = "vh" ) + bgcolor("white"),
          width = 55, height = 32, unit = "cm")
...
### GSEA celltypesMain (GO_Biological_Process_2021 and MSigDB_Hallmark_2020)
...
mut_stat$mut <- ifelse(mut_stat$mean < 0.02, "unmutated", "mutated")
...
saveRDS(DGE_list, "../results/DGE_B-cells_celltypeMain_mutationStatus_listAll.rds")
#DGE_list <- readRDS("../results/DGE_B-cells_celltypeMain_patientGroups_listAll.rds")
...
saveRDS(DGE_list, "../results/DGE_B-cells_celltypeFine_mutationStatus_listAll.rds")
#DGE_list <- readRDS("../results/DGE_B-cells_celltypeMain_patientGroups_listAll.rds")

for (i in names(DGE_list)){
  print(i)
  print(nrow(head(DGE_list[[i]])))
  print(nrow(DGE_list[[i]][DGE_list[[i]]$FDR < 0.1, ]))
  print(head(head(DGE_list[[i]])))
}
