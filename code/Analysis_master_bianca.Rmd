...
samples <- str_replace(str_sort(list.files("../data/counts_mx"), 
                                numeric = TRUE), 
                       "_filtered_feature_bc_matrix", ""); samples

####----- Create a named vector with paths to samples
samples.dirs <- sapply(samples, function(i){
  d10x <- file.path('../data/counts_mx', paste0(i, '_filtered_feature_bc_matrix'))
  d10x
}); samples.dirs
...
gex_data <- Read10X(data.dir = samples.dirs,
                    ...
                    strip.suffix = TRUE); str(gex_data); mem_used() #3.99 GB
...
gex <- CreateSeuratObject(gex_data, 
                          ...
                          min.features = 0,
                          names.field = 1)
table(gex@meta.data$orig.ident); nrow(gex@meta.data); mem_used() #4 GB

####-----  rm tmp objects
rm(samples, samples.dirs, gex_data); mem_used()
...
#scDblFinder requires like 3 min for this data set
...
write.csv(discarded_stats, paste0("../suppl/stats_discarded_",format(Sys.time(), "%y%m%d"),".csv"), 
          row.names = FALSE, 
          quote = FALSE)
...
ig.features <- c("IGHA1", "IGHA2", "IGHG1", "IGHG2", "IGHG3", "IGHG4",  "IGHD", "IGHE", "IGHM" )
b.markers <- c("CD79A", "CD79B", "MS4A1", "CD19", "CD27", "IGHA", "IGHD", "IGHM", "JCHAIN", "MME")
pbmc.markers <- c("MS4A1", "CD19", "CD27", "CD79A", "GNLY", "CD3E", "CD14", "LYZ", "CD8A")
my_pars <- c("nCount_RNA", "nFeature_RNA", "percent_mito", "percent_ribo", "percent_hb")
...
detable <- read.csv("../results/DGE_B-cells_seurat_clusters_210628.csv")
head(detable, n=25 )
