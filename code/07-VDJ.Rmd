...
#### plot SFig10A, allele usage for differentially used IGHV genes IGHV1-69
...
#### plot SFig10B?, VDJ gene combination usage per patient group for IGHV1-69
...
m <- unique(gex.meta[(gex.meta$cluster_no_airr %in% rownames(tmp2)) & 
...
#res <- res[c(1:100, 900:1000, 2400:2500, 3500:3600), c(1:100, 900:1000, 2400:2500, 3500:3600)] 
...
#annotation <- data.frame(group = factor(m$group[c(1:100, 900:1000, 2400:2500, 3500:3600)]), row.names = m$CDR3[c(1:100, 900:1000, 2400:2500, 3500:3600)])
...
for (i in c("png")){
  ...
  for (j in unique(m$group)[c(2, 3, 1, 4:6)]){
    ...
    p_list[[j]] <- pheatmap(res[, which(m$group %in% j)],
            ...
            annotation_col = data.frame(group = factor(m$group), row.names = m$CDR3),
            annotation_colors = ann_cols[1],
            filename = paste0("../results/Fig4D", "_", j, ".", i),
            clustering_method = "ward.D2",
            legend = TRUE,
            show_rownames = FALSE, 
            show_colnames = FALSE, 
...
corder <- c(p_list$`SSA-`$tree_col$labels[p_list$`SSA-`$tree_col$order],
  ...
  p_list$Corsiero_et_al$tree_col$labels[p_list$Corsiero_et_al$tree_col$order],
  p_list$Visser_et_al$tree_col$labels[p_list$Visser_et_al$tree_col$order],
  p_list$Wang_et_al$tree_col$labels[p_list$Wang_et_al$tree_col$order])
  
h1 <- pheatmap(res[corder, corder], 
           show_rownames = FALSE, 
           show_colnames = FALSE, 
           filename = paste0("../results/Fig4D_test_.", "png"),
           annotation_col = data.frame(group = factor(m$group), row.names = m$CDR3),
           annotation_row = data.frame(group = factor(m$group), row.names = m$CDR3),
           annotation_colors = ann_cols[1], 
           legend = TRUE,
...
# cl1 <- paste0("SSA-", cutree(p_list$`SSA-`$tree_col, k = 4 ))
# cl2 <- paste0("SSA+", cutree(p_list$`SSA+`$tree_col, k = 4 ))
# cl3 <- paste0("SSAB", cutree(p_list$SSAB$tree_col, k = 4 ))
# cl4 <- paste0("Corsiero", cutree(p_list$Corsiero_et_al$tree_col, k = 3 ))
# cl5 <- paste0("Visser",cutree(p_list$Visser_et_al$tree_col, k = 3 ))
# cl6 <- paste0("Wang", cutree(p_list$Wang_et_al$tree_col, k = 1 ))
...
cl4 <- paste0("Corsiero", cutree(p_list$Corsiero_et_al$tree_col, k = 1 ))
cl5 <- paste0("Visser",cutree(p_list$Visser_et_al$tree_col, k = 1 ))
cl6 <- paste0("Wang", cutree(p_list$Wang_et_al$tree_col, k = 1 ))
...
o <- c(p_list$`SSA-`$tree_col$labels,
       ...
       p_list$Corsiero_et_al$tree_col$labels,
       p_list$Visser_et_al$tree_col$labels,
       p_list$Wang_et_al$tree_col$labels)

res2 <- rowsum(res[o, o], cl_all)/c(table(cl_all))
res2 <- rowsum(t(res2), cl_all)/c(table(cl_all))
res2 <- res2[ c(2:4, 1, 5:6), c(2:4, 1, 5:6)]

h1 <- pheatmap(res2, 
           show_rownames = TRUE, 
           show_colnames = TRUE, 
           filename = paste0("../results/Fig4D_test.", "png"),
           #annotation_col = data.frame(group = factor(colnames(res2)), row.names = colnames(res2)),
           #annotation_row = data.frame(group = factor(colnames(res2)), row.names = colnames(res2)),
           #annotation_colors = ann_cols[1], 
           legend = TRUE,
           use_raster = TRUE,
           cluster_rows = FALSE,
