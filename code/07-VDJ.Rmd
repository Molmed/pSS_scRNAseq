...
#Get average percent IGHM in classical memory B cells per patient group
cell_summary %>% filter(cluster_cellType_manual_main == "Memory_Classical", c_gene_10x == "IGHM") %>% group_by(patient_group) %>% summarize(Mean = mean(per, na.rm=TRUE))

#ANOVA for the average percent IGHM in classical memory B cells per patient group
tmp <- cell_summary %>% filter(cluster_cellType_manual_main == "Memory_Classical", c_gene_10x == "IGHM") %>% group_by(patient_group) 
aov.model <- aov(tmp$per ~ tmp$patient_group)
summary(aov.model)
...
#Get statistical support for observed IGHV differences
vdj_gene <- countGenes(gex@meta.data, 
                       gene = "v_call", 
                       groups = c("orig.ident", "patient_group"), 
                       mode = "gene",
                       fill = FALSE)

for (i in c("IGHV1-69", "IGHV3-30-3", "IGHV4-30-4", "IGHV4-39", "IGHV3-66", "IGHV4-34")){
  ...
  tmp <- vdj_gene %>% 
            ...
            summarize(Mean = mean(seq_freq, na.rm=TRUE))
  print(tmp)
  
  #ANOVA 
  tmp <- vdj_gene %>% filter(gene == i) %>% group_by(patient_group) 
  aov.model <- aov(tmp$seq_freq ~ tmp$patient_group)
...
#check IGHV1-69 allele significance between groups
vdj_allele_tmp <- countGenes(gex@meta.data, 
                       gene = "v_call", 
                       groups = c("patient_group", "orig.ident"), 
                       mode = "allele",
                       fill = TRUE)

#ANOVA 
tmp <- vdj_allele_tmp[grepl("^IGHV1-69*", vdj_allele_tmp$gene), ] %>% 
  filter(gene == "IGHV1-69*01") %>% group_by(patient_group) %>% ggplot(aes(y = seq_freq, x = patient_group)) + geom_boxplot()
aov.model <- aov(tmp$seq_freq ~ tmp$patient_group)
summary(aov.model) #0.0814
...
tmp <- as.data.frame.matrix(table(gex.meta$mu_load, gex.meta$cluster_cellType_manual_main))

#percent "Naive"" cells with unmutated transcripts
sum(tmp["unmutated", c("Naive", "Transitional")])/sum(tmp[, c("Naive", "Transitional")]) #[1] 0.9041047

#percent "Memory"" cells with mutated transcripts
1-(sum(tmp["unmutated", c("Memory_Classical", "Memory_DN", "Memory_IgM")])/sum(tmp[, c("Memory_Classical", "Memory_DN", "Memory_IgM")])) #[1] 0.7901087

cell_summary <-  as_tibble(gex.meta) %>% 
  filter(!is.na(c_gene_10x)) %>%
  select(orig.ident, cluster_cellType_manual_main, patient_group, mu_load) %>%
  add_count(orig.ident, cluster_cellType_manual_main, mu_load, .drop = FALSE) %>% 
  distinct() %>%
  group_by(orig.ident, cluster_cellType_manual_main) %>%
  mutate(per =  prop.table(n)) %>%
  drop_na()
...
#Get average percent IGHM in classical memory B cells per patient group
cell_summary %>% 
  filter(cluster_cellType_manual_main == "Memory_Classical" |
                        cluster_cellType_manual_main == "Memory_IgM"|
                        cluster_cellType_manual_main == "Memory_DN",
                        mu_load == "unmutated") %>% 
  group_by(patient_group) %>% 
  summarize(Mean = mean(per, na.rm=TRUE))

cell_summary %>% 
  filter(cluster_cellType_manual_main == "Memory_IgM",
                        mu_load == "unmutated") %>% 
  group_by(patient_group) %>% 
  summarize(Mean = 1-mean(per, na.rm=TRUE))


#ANOVA for the average percent IGHM in classical memory B cells per patient group
tmp <- cell_summary %>% filter(cluster_cellType_manual_main == "Memory_Classical" |
                                cluster_cellType_manual_main == "Memory_IgM"|
                                cluster_cellType_manual_main == "Memory_DN",
                                mu_load == "unmutated") %>% 
  group_by(patient_group) 
...
cell_summary <-  as_tibble(gex.meta) %>% 
  filter(!is.na(mu_count_cdr3_RSratio)) %>%
  select(orig.ident, cluster_cellType_manual_main, patient_group, mu_count_cdr3_RSratio) %>% 
  group_by(patient_group) %>% 
  summarize(Mean = 2^mean(mu_count_cdr3_RSratio, na.rm=TRUE))
