...
#### plot SFig12A-C top VDJ genes per patient subgroup
...
ggsave2(paste0("../results/SFig12.png"), 
          plot_grid(pd, pj, align = "hv", labels = "AUTO", ncol = 1) +
          bgcolor("white") + 
          border("white"),
...
ggsave2(paste0("../results/SFig11.png"), 
...
gex@meta.data$IGHV1_69_umi[which(gex@meta.data$IGHV_gene == "IGHV1-69")] <- gex@meta.data$umi_count[which(gex@meta.data$IGHV_gene == "IGHV1-69")]
...
tmp <- as.data.frame.matrix(table(gex@meta.data$IGHV_gene, gex@meta.data$cluster_cellType_manual_fine))
gex@meta.data$cluster_cellType_manual_fine <- as.factor(gex@meta.data$cluster_cellType_manual_fine)

p <- as_tibble(gex@meta.data) %>% 
  select(orig.ident, cluster_cellType_manual_fine, IGHV_gene, patient_group) %>% 
  filter(IGHV_gene == "IGHV1-69")  %>%  
...
gex.meta$IGHV_gene <- as.factor(gex.meta$IGHV_gene)
gex.meta$orig.ident <- as.factor(gex.meta$orig.ident)

p <- select(gex.meta, orig.ident, cluster_cellType_manual_fine, IGHV_gene) %>% 
  group_by(orig.ident, cluster_cellType_manual_fine, IGHV_gene, .drop = FALSE) %>%
  summarise(n = n()) %>%
...
p <- as_tibble(gex@meta.data) %>% 
  select(orig.ident, cluster_cellType_manual_main, IGHV_gene, patient_group) %>% 
  filter(IGHV_gene == "IGHV1-69")  %>%  
...
# !!!! fix this !!!!
p <- as_tibble(gex@meta.data) %>% 
  select(cluster_cellType_manual_main, IGHV_gene, patient_group) %>% 
  filter(IGHV_gene == "IGHV1-69")  %>%  
...
ggsave2(paste0("../results/SFig17_2.png"),
...
####---- save pies
ggsave2(paste0("../results/Fig4B_2.png"),
          plot_grid(p4, p_legend, 
                    ncol = 1, 
                    labels = NULL, 
                    align = "v", 
...
#### plot Fig4C_I venn diagram of clonotypes (1640)
...
venn.diagram(venn.list, 
             filename = "../results/Fig5B_I_2.png", 
             na = "none", 
...
#### plot Fig4C_II UpSetR of clonotypes (1640)

```{R}

png("../results/Fig4C_II_3.png", width = 12, height = 11, units = "cm", res = 200)
...
#####---- show that there's no overlap between CDR3 sequences from the three data sets
CDR3_data <- unique(CDR3_data[, c("CDR3", "dataset")])
colnames(CDR3_data) <- c("CDR3", "group")
                
CDR3_counts <- as.data.frame.matrix(table(CDR3_data$CDR3,
                                  CDR3_data$group))
CDR3_counts$ident <- as.character(rownames(CDR3_counts))

venn.list <- list()[1:3]
names(venn.list) <- colnames(CDR3_counts)[1:3]
for (i in colnames(CDR3_counts)[1:3]){
  for (j in 1:nrow(CDR3_counts))
  venn.list[[i]][j] <- ifelse(CDR3_counts[j, i] == 1, CDR3_counts$ident[j], NA)
  #venn.list[[i]][venn.list[[i]] == "0"] <- NA
}
venn.list <- lapply(venn.list, function(x){x <- na.omit(x)})

venn.diagram(venn.list, 
             filename = "../results/SFig23_2.png", 
             na = "none", 
             print.mode = c("raw", "percent"), 
             category.names = names(venn.list),
             disable.logging = TRUE,
...
####------- pairwise levenstein distances between public data sets

# corsiero vs visser
length(CDR3_data$CDR3[CDR3_data$group == "corsiero"]) # number of unique CDR3 in data set:  347
length(CDR3_data$CDR3[CDR3_data$group == "visser"]) # number of unique CDR3 in data set: 98
res <- stringdist::stringdistmatrix(CDR3_data$CDR3[CDR3_data$group == "corsiero"], 
                                    CDR3_data$CDR3[CDR3_data$group == "visser"],  method = "lv")
res <- as.data.frame(Matrix(res, sparse = T, dimnames = list(CDR3_data$CDR3[CDR3_data$group == "corsiero"], 
                                                             CDR3_data$CDR3[CDR3_data$group == "visser"])))

res_min <- apply(res, 1, min)
min(res_min) #min levenstein distance: 7
length(names(res_min)[res_min == min(res_min)]) # number of CDR3 with min levenstein distance :2
res[order(res_min, decreasing = F), ]

# corsiero vs wang
length(CDR3_data$CDR3[CDR3_data$group == "corsiero"]) # number of unique CDR3 in data set:  347
length(CDR3_data$CDR3[CDR3_data$group == "wang"]) ## number of unique CDR3 in data set: 12
res <- stringdist::stringdistmatrix(CDR3_data$CDR3[CDR3_data$group == "corsiero"], 
                                    CDR3_data$CDR3[CDR3_data$group == "wang"],  method = "lv")
res <- as.data.frame(Matrix(res, sparse = T, dimnames = list(CDR3_data$CDR3[CDR3_data$group == "corsiero"], 
                                                             CDR3_data$CDR3[CDR3_data$group == "wang"])))

res_min <- apply(res, 1, min)
min(res_min) #min levenstein distance: 7
length(names(res_min)[res_min == min(res_min)]) # number of CDR3 with min levenstein distance :4
res[order(res_min, decreasing = F), ]
...
length(CDR3_data$CDR3[CDR3_data$group == "visser"]) # number of unique CDR3 in data set: 98
length(CDR3_data$CDR3[CDR3_data$group == "wang"]) ## number of unique CDR3 in data set: 12
res <- stringdist::stringdistmatrix(CDR3_data$CDR3[CDR3_data$group == "visser"], 
                                    CDR3_data$CDR3[CDR3_data$group == "wang"],  method = "lv")
res <- as.data.frame(Matrix(res, sparse = T, dimnames = list(CDR3_data$CDR3[CDR3_data$group == "visser"], 
                                                             CDR3_data$CDR3[CDR3_data$group == "wang"])))

res_min <- apply(res, 1, min)
min(res_min) #min levenstein distance: 6
length(names(res_min)[res_min == min(res_min)]) # number of CDR3 with min levenstein distance :1
res[order(res_min, decreasing = F), ]
...
#### dist between present study and litterature

```{r}
tmp <- as.data.frame.matrix(table(gex.meta$cluster_no_airr,
                                  gex.meta$orig.ident))

#get clonotypes present in ??? 4 samples
...
m <- unique(gex.meta[(gex.meta$cluster_no_airr %in% rownames(tmp2)) & !gex.meta$patient_group == "CTRL", 
                     c("junction_aa", "patient_group")])
m <- m[!duplicated(m$junction_aa),]
colnames(m) <- c("CDR3", "group")
dim(m)

# vs corsiero
length(m$CDR3) # number of unique CDR3 in data set: 4092
length(CDR3_data$CDR3[CDR3_data$group == "corsiero"]) ## number of unique CDR3 in data set: 347
res <- stringdist::stringdistmatrix(m$CDR3, 
                                    CDR3_data$CDR3[CDR3_data$group == "corsiero"],  method = "lv")
res <- as.data.frame(Matrix(res, sparse = T, dimnames = list(m$CDR3, 
                                                             CDR3_data$CDR3[CDR3_data$group == "corsiero"])))

res_min <- apply(res, 1, min)
min(res_min) #min levenstein distance: 3
length(names(res_min)[res_min == min(res_min)]) # number of CDR3 with min levenstein distance :2
res[order(res_min, decreasing = F), ]
...
res_min <- apply(res, 1, min)
min(res_min) #min levenstein distance: 2
length(names(res_min)[res_min == min(res_min)]) # number of CDR3 with min levenstein distance :8
res[order(res_min, decreasing = F), ]
...
res_min <- apply(res, 1, min)
min(res_min) #min levenstein distance: 4
length(names(res_min)[res_min == min(res_min)]) # number of CDR3 with min levenstein distance :8
res[names(res_min)[res_min == min(res_min)], ]
res[order(res_min, decreasing = F), ]
...
res <- as.data.frame(Matrix(res, sparse = T, dimnames = list(m$CDR3, m$CDR3)))
...
ann_cols <- list(group = c(corsiero = brewer.pal(9, "YlOrRd")[2],
                           ...
                           SSA = "#00BFC4", 
                           SSAB = "#C77CFF"))
ann_cols <- list(group = c(`Corsiero et al.` = brewer.pal(9, "YlOrRd")[2],
                           `Visser et al.` = brewer.pal(9, "YlOrRd")[5],  
                           `Wang et al.` = brewer.pal(9, "YlOrRd")[7], 
                           DNEG = "#7CAE00",
                           SSA = "#00BFC4", 
                           SSAB = "#C77CFF"))
annotation <- data.frame(group = factor(m$group), row.names = m$CDR3 )
length(m$CDR3)
length(unique(m$CDR3))
m[duplicated(m$CDR3), ]


# m[m$CDR3 %in% unique(m$CDR3[ duplicated(m$CDR3)]), "CDR3"]
# table(m[m$CDR3 %in% unique(m$CDR3[ duplicated(m$CDR3)]), "CDR3"], m[m$CDR3 %in% unique(m$CDR3[ duplicated(m$CDR3)]), "group"])

pheatmap(res, 
         show_rownames = FALSE, 
         show_colnames = FALSE, 
         filename = "../results/Fig4D_6.png",
         annotation_col = data.frame(group = factor(m$group), row.names = m$CDR3),
         annotation_row = data.frame(group = factor(m$group), row.names = m$CDR3),
         annotation_colors = ann_cols[1], legend = T
         )
dev.off()
...
#### plot SFig14A , CDR3 mutation count ratio R/S - cell type fine - patient_group,
...
#### plot SFig14B, fraction of CDR3 mutated cells per sample and celltypeFine
...
### plot SFig10A, IgH class per cellTypeMain per patient
...
ggsave2(paste0("../results/SFig10A.png"), 
          plot_grid(p, p_legend, 
                    ncol = 1, labels = NULL, 
...
### plot SFig10B, IgH class per cellTypeFine per patient
...
ggsave2(paste0("../results/SFig10B.png"), 
          plot_grid(p2, p_legend, ncol = 1, labels = NULL, align = "hv", rel_heights = c(1, 0.1)) +
          bgcolor("white") + 
          border("white"),
          width = 30, height = 25, unit = "cm")
