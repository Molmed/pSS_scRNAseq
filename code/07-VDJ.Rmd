...
#gex$VDJ contains concatenated VDJ gene names
...
p3 <- plot_grid(p1, p2, nrow = 2, labels = NULL,
 ...
 ggsave2(paste0("../results/Fig3D.png"),
          plot_grid(p3, p_legend, 
                    ncol = 2, 
                    labels = NULL, 
                    align = "v", 
...
### plot Fig4D heatmap of top clonotypes

```{r}

####--------------------------------------------------------------------- 
####---- get clonotypes present in ≥ 4 samples using output from compairr
####---------------------------------------------------------------------
...
gex.meta$VDJ
names(head(sort(table(gex.meta$junction_aa), decreasing = T), n=10))


gex.meta[gex.meta$junction_aa %in% names(head(sort(table(gex.meta$junction_aa), decreasing = T), n=10))[1]]

gex.meta$cluster_cellType_manual_main[gex.meta$junction_aa %in% names(head(sort(table(gex.meta$junction_aa), decreasing = T), n=10))[2]]

gex.meta$orig.ident[gex.meta$junction_aa %in% names(head(sort(table(gex.meta$junction_aa), decreasing = T), n=100))[100]]

gex.meta$orig.ident[gex.meta$junction_aa %in% CDR3_data$CDR3]

table(gex.meta$junction_aa %in% CDR3_data$CDR3)
...
#gex.meta.tmp <- gex.meta[match(rownames(test)[1:15], gex.meta$cluster_no_airr), ]
gex.meta.tmp <- gex.meta[gex.meta$cluster_no_airr %in% rownames(test)[1:15], ]
gex.meta.tmp <- gex.meta[gex.meta$cluster_no_airr %in% rownames(test), ]
unique(gex.meta.tmp$junction_aa[gex.meta.tmp$cluster_no_airr == "20"])

nrow(gex.meta.tmp)
sort(table(gex.meta.tmp$VDJ))
sort(table(gex.meta.tmp$orig.ident))
length(sort(table(gex.meta.tmp$orig.ident)))
sort(names(sort(table(gex.meta.tmp$orig.ident))))
...
pheatmap(res, 
         show_rownames = FALSE, 
         show_colnames = FALSE, 
         filename = "../results/clonotypes_lvDist_test.png")
dev.off()
...
#all datasets
res <- stringdist::stringdistmatrix(unique(gex.meta$junction_aa), unique(CDR3_data$CDR3),  method = "lv")
res <- as.data.frame(Matrix(res, sparse = T, dimnames = list(unique(gex.meta$junction_aa), unique(CDR3_data$CDR3))))
...
res_min <- apply(res, 1, min)
res_min <- res_min[order(res_min)]
length(res_min[res_min <= 2])

r_names <- names(res_min[res_min <= 2])
r_names <- r_names [!is.na(r_names )]

test <- res[r_names,]


c_names <- unique(apply(test, 1, function(x){
  colnames(test)[x == 2]
}))
...
CDR3_data[CDR3_data$CDR3 %in% colnames(test),]
gex.meta[gex.meta$junction_aa %in% rownames(test), c("orig.ident", "patient_group", "VDJ", "cluster_cellType_manual_fine", "mu_load")]
...
### plot Fig4C_I venn diagram of clonotypes (1799)
...
### plot Fig4C_II UpSetR of clonotypes (1799)
...
####  plot Fig3E, mutation load per cell type
...
#### plot Fig3F, total VDJ mutation frequency - celltypeFine - patient_group
...
#### plot SFig12A , CDR3 mutation count ratio R/S - cell type fine - patient_group,
...
#### plot SFig12B, fraction of CDR3 mutated cells per sample and celltypeFine
...
#### plot SFig13A, total VDJ mutation frequency - celltypeMain - orig.ident 
...
####  plot SFig13B, total VDJ mutation frequency - celltypeFine - orig.ident
...
##  plot Fig4A cloneType per cell type fine based on compairr data
...
## plot SFig14A, IgH class per cellTypeMain per patient
...
ggsave2(paste0("../results/SFig14A.png"), 
          plot_grid(p, p_legend, 
                    ncol = 1, labels = NULL, 
...
## plot SFig14B, IgH class per cellTypeFine per patient
...
ggsave2(paste0("../results/SFig14B.png"), 
          plot_grid(p2, p_legend, ncol = 1, labels = NULL, align = "hv", rel_heights = c(1, 0.1)) +
          bgcolor("white") + 
          border("white"),
          width = 30, height = 25, unit = "cm")
...
##  plot SFig15, CDR3 aa attributes
...
## plot SFig16A, clonotypes per patient per cellTypeMain
...
ggsave2(paste0("../results/SFig16A.png"), 
          plot_grid(p, p_legend, 
                    ncol = 1, labels = NULL, 
                    align = "hv", rel_heights = c(1.5, 0.2)) + 
          bgcolor("white") + 
...
## plot SFig16B, clonotypes per patient per cellTypeFine
...
ggsave2(paste0("../results/SFig16B.png"), 
          plot_grid(p2, p_legend, ncol = 1, labels = NULL, align = "hv", rel_heights = c(1, 0.1)) +
          bgcolor("white") + 
          border("white"),
          width = 30, height = 25, unit = "cm")
