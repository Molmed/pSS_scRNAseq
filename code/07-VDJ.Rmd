...
, "c_call"
, size = 2, alpha = 0.8
 group = c("patient_group")

 
clones <- countClones(outfile )
arrange(clones, patient_group, seq_freq) %>% 
ggplot(aes(x = clone_id, y = seq_freq)) +
      theme_bw() +
      geom_bar(aes(color = patient_group))
...
tmp <- table(list(gex$v_call, gex$seurat_clusters_patient_group))
head(tmp)
cor_tmp <- cor(tmp, method = "spearman")
head(cor_tmp)
cor_tmp[is.na(cor_tmp)] <- 0
aaa <- factor(sub("_.*","c",colnames(cor_tmp)))
library(pheatmap)

head(gex@meta.data)
pheatmap::pheatmap(cor_tmp, annotation_col =  
                     data.frame(cluster = factor(sub("_.*","",colnames(cor_tmp))),
                                patient_group = factor(sub(".*_","",colnames(cor_tmp))),
                                row.names = colnames(cor_tmp)),
                   breaks = seq(-1,1,length.out = 100),
                   filename = "../results/v_cll_cor_per_cluster_Patient_Group.pdf",
                   color = colorRampPalette(c("navy", "gray95", "firebrick"))(99) )

pheatmap::pheatmap(log2(tmp + 1), annotation_col =  
                     data.frame(cluster = factor(sub("_.*","",colnames(cor_tmp))),
                                patient_group = factor(sub(".*_","",colnames(cor_tmp))),
                                row.names = colnames(cor_tmp)), 
                   filename = "../results/v_call_per_cluster_Patient_Group.pdf",
                   #breaks = seq(-1,1,length.out = 100),
                   color = c( "gray95", colorRampPalette(c("gray80", "orange3", "firebrick"))(98)) )
