...
VDJ_cor <- function(i, #seurat object containing meta data for VDJ gene usage
  ...
  i@meta.data[, VDJgene] <- unlist(lapply(strsplit(i@meta.data[, VDJgene], 
                                                   split = "\\*"),
  ...
  #cat(cor.test(tmp, method = correl))
...
p <- list()
for (i in sort(unique(gex$cluster_cellType_manual_fine))){
p[[i]] <- ggplot(gex@meta.data[gex@meta.data$cluster_cellType_manual_fine == i, ], 
             aes(x = patient_group, 
                 y = mu_freq_total, 
                 fill = orig.ident)) +
      theme_classic() +
...
ggsave2(paste0("../results/VDJ_muFull_celltypeFine_orig.ident.png"), 
          plot_grid(plotlist = p, ncol = 4, labels = NULL, align = "hv"),
