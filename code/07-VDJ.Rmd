...
table(gex$cluster_cellType_manual_main, gex$cluster_cellType_manual_fine)
...
pheatmap(tmp,
         ...
         filename = "../results/VDJ_clono_clonotypeDistribution_perSample_topClonotypes75_1.png"
         )  
...
# write.csv(outfile, "../results/1799_clonotypes.csv", sep = ",", col.names = TRUE, row.names = FALSE, quote = FALSE)

colSums(as.data.frame.matrix(table(outfile$cluster_no_airr, outfile$orig.ident)))
#make the data frame binary
tmp <- (tmp > 0) * 1
...
plot(as.dendrogram(h.3))
abline(h = 0.2, col ="red")
#cl <- cutree(h.3, k = 15)
clr <- cutree(h.3, h = 0.2)

tmp4 <- rowsum(tmp, clr)
h.4 <- hclust(dist(t(tmp4), method = "binary"), method = "complete")
plot(as.dendrogram(h.4))
abline(h = 0.2, col ="red")
clc <- cutree(h.4, h = 0.2)
...
p2 <- pheatmap(tmp2,
         ...
         labels_row = annot_junc$junction_aa[match(rownames(tmp2), 
                                                   annot_junc$cluster_no_airr)],
         filename = "../results/VDJ_clono_clonotypeDistribution_IGHV_topClonotypes.png"
         )  
...
xxx <- annot_junc$junction_aa[annot_junc$cluster_no_airr %in% rownames(tmp)]
...
plot(as.dendrogram(h.5))

abline(h = 0.2, col ="red")
dev.off()
class(d)
str(d)
row.names(as.matrix(d))
names(d)
d
head(h.3)
head(h.3$labels)
...
gex.meta$junction_aa[gex.meta$cluster_no_airr %in% names(clr[clr == 14])]
...
p3 <- pheatmap(as.matrix(d),
         ...
         filename = "../results/VDJ_clono_clonotypeDistribution_hamm.dist_5.png",
         scale = "row",
...
gex.meta$mu_count_cdr3_RSratio <- (gex.meta$mu_count_cdr3_r + 1) / (gex.meta$mu_count_cdr3_s + 1)
...
p <- ggplot(gex.meta, 
             aes(x = cluster_cellType_manual_fine, 
                 ...
                 fill = patient_group)) +
      theme_classic() +
      ggtitle("VDJ mutations, CDR3 - R / S") +
      theme(axis.text.x = element_text(angle = 45, 
                                       hjust = 1, vjust = 1)) +
      ylab("mutation count ratio R/S") +
      xlab("") +
      #scale_y_continuous(labels = percent) +
...
gex.meta$mu_count_cdr_RSratio <- (gex.meta$mu_count_cdr_r + 1) / (gex.meta$mu_count_cdr_s + 1)
...
gex.meta$mu_count_fwr_RSratio <- (gex.meta$mu_count_fwr_r + 1) / (gex.meta$mu_count_fwr_s + 1)

p <- ggplot(gex.meta, 
             aes(x = cluster_cellType_manual_fine, 
                 y = mu_count_fwr_RSratio, 
                 fill = patient_group)) +
      theme_classic() +
      ggtitle("VDJ mutation count ratio R/S, FWR1-4") +
      theme(axis.text.x = element_text(angle = 45, 
                                       hjust = 1, vjust = 1)) +
      ylab("mutation count ratio R/S") +
      xlab("") +
...
ggsave2(paste0("../results/VDJ_muFull_FWR_countRatio_celltypeFine_patientGroup.png"), 
          p,
          width = 30, height = 15, unit = "cm")
...
####---- total VDJ mutation frequency - celltypeFine - orig.ident (with pat labels)
p <- list()
for (i in sort(unique(gex.meta$cluster_cellType_manual_fine))){
p[[i]] <- ggplot(gex.meta[gex.meta$cluster_cellType_manual_fine == i, ], 
             aes(x = orig.ident, 
...
p_plots <- plot_grid(plotlist = p, ncol = 4, labels = NULL, align = "hv")

ggsave2(paste0("../results/VDJ_muFull_celltypeFine_orig.ident_3.png"), 
          plot_grid(p_plots, p_legend, 
                    ncol = 2, 
                    rel_widths = c(10, 1), 
                    labels = NULL, 
                    align = "hv") + bgcolor("white"),
