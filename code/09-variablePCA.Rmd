---
output: html_document
editor_options: 
  chunk_output_type: console
---
# PCA per sample variables {#PCA}

## Load packages

```{r}

source("./00a_dependencies.R")

```


## Load Seurat object

```{r}

gex <- readRDS("../results/GEX7_BVDJ.rds")

```


## reduce gex size

```{r}
# gex.meta <- gex@meta.data[(gex$cluster_cellType_manual_fine == "DN1_ID3" |
#                           gex$cluster_cellType_manual_fine == "DN2" |
#                           gex$cluster_cellType_manual_fine == "DN2_CXCR3" |
#                           gex$cluster_cellType_manual_fine == "DN2_ITGAX" |
#                           gex$cluster_cellType_manual_fine == "DN4" |
#                           gex$cluster_cellType_manual_fine == "Memory_Cl" |
#                           gex$cluster_cellType_manual_fine == "Memory_IgM" |
#                           gex$cluster_cellType_manual_fine == "Memory_IgM_ALOX5" |
#                           gex$cluster_cellType_manual_fine == "Memory_IgM_CD1C" |
#                           gex$cluster_cellType_manual_fine == "Naive" |
#                           gex$cluster_cellType_manual_fine == "Naive_IFN" |
#                           gex$cluster_cellType_manual_fine == "Naive_Transitional"), ]

gex.slim <- gex; rm(gex)
gex.slim@assays$RNA@scale.data <- matrix()
to_remove <- c("pca_1", "pca_2", "scpred", "scpred_projection", 
               "harmony_1", "harmony_2", "umap_1", "umap_2_3d")
gex.slim@reductions <- gex.slim@reductions[!(names(gex.slim@reductions) %in% to_remove)]

gex.slim <- gex.slim[, (gex.slim$cluster_cellType_manual_fine == "DN1_ID3" |
                        gex.slim$cluster_cellType_manual_fine == "DN2" |
                        gex.slim$cluster_cellType_manual_fine == "DN2_CXCR3" |
                        gex.slim$cluster_cellType_manual_fine == "DN2_ITGAX" |
                        gex.slim$cluster_cellType_manual_fine == "DN4" |
                        gex.slim$cluster_cellType_manual_fine == "Memory_Cl" |
                        gex.slim$cluster_cellType_manual_fine == "Memory_IgM" |
                        gex.slim$cluster_cellType_manual_fine == "Memory_IgM_ALOX5" |
                        gex.slim$cluster_cellType_manual_fine == "Memory_IgM_CD1C" |
                        gex.slim$cluster_cellType_manual_fine == "Naive" |
                        gex.slim$cluster_cellType_manual_fine == "Naive_IFN" |
                        gex.slim$cluster_cellType_manual_fine == "Naive_Transitional")]


```


## Patient Meta Data

```{r}

source("../results/00b_pheno.R")

pheno <- pheno[-28,] # remove P024

#make binary data columns numerical binary
pheno[pheno == "NO"] <- 0
pheno[pheno == "YES"] <- 1
pheno[pheno == "MD"] <- NA
pheno[, 2:(ncol(pheno)-1)] <- sapply(pheno[, 2:(ncol(pheno)-1)], as.numeric)

#remove variables only present in one sample
pheno <- pheno[, c(1, (1 + which(colSums(pheno[,2:29], na.rm = TRUE) > 2, )), 34)]


```


## celltype frequencies per patient

```{r}

#cellType_manual_fine
cells_fine <- dplyr::select(gex.slim@meta.data,
                   orig.ident,
                   cluster_cellType_manual_fine) %>%
  add_count(orig.ident, 
            cluster_cellType_manual_fine, 
            .drop = FALSE) %>%
  distinct() %>%
  group_by(orig.ident) %>%
  mutate(per =  prop.table(n)*100) %>%
  drop_na() %>%
  dplyr::select(orig.ident, 
                cluster_cellType_manual_fine, 
                per) %>%
  pivot_wider(names_from = cluster_cellType_manual_fine, 
              values_from = per)

colnames(cells_fine)[2:ncol(cells_fine)] <- paste0(colnames(cells_fine)[2:ncol(cells_fine)], "_%")


cells_main <- dplyr::select(gex.slim@meta.data,
                       orig.ident,
                       cluster_cellType_manual_main) %>%
  add_count(orig.ident,
            cluster_cellType_manual_main,
            .drop = FALSE) %>%
  distinct() %>%
  group_by(orig.ident) %>%
  mutate(per =  prop.table(n)*100) %>%
  drop_na() %>%
  dplyr::select(orig.ident,
                cluster_cellType_manual_main,
                per) %>%
  pivot_wider(names_from = cluster_cellType_manual_main,
              values_from = per)

colnames(cells_main)[2:ncol(cells_main)] <- paste0(colnames(cells_main)[2:ncol(cells_main)], "_%")

# celltypes <- inner_join(cells_main, cells_fine, by = "orig.ident") 


```


## VDJ percentages per celltype

```{r}

# IGHV
IGHV <- dplyr::select(gex.slim@meta.data,
                           orig.ident,
                           IGHV) %>%
  add_count(orig.ident,
            IGHV,
            .drop = FALSE) %>%
  distinct() %>%
  group_by(orig.ident) %>%
  mutate(per =  prop.table(n)*100) %>%
  drop_na() %>%
  dplyr::select(orig.ident, IGHV, per) %>%
  pivot_wider(
    names_from = c(IGHV),
    values_from = per,
    values_fill = 0
  )

colnames(IGHV)[2:ncol(IGHV)] <-
  paste0(colnames(IGHV)[2:ncol(IGHV)], "_%")

# # IGHV gene per celltypeFine
# IGHV_fine <- dplyr::select(gex.slim@meta.data,
#                            orig.ident,
#                            cluster_cellType_manual_fine,
#                            IGHV) %>%
#   add_count(orig.ident,
#             cluster_cellType_manual_fine,
#             IGHV,
#             .drop = FALSE) %>%
#   distinct() %>%
#   group_by(orig.ident,
#            cluster_cellType_manual_fine) %>%
#   mutate(per =  prop.table(n)*100) %>%
#   drop_na() %>%
#   dplyr::select(orig.ident, cluster_cellType_manual_fine, IGHV, per) %>%
#   pivot_wider(
#     names_from = c(IGHV, cluster_cellType_manual_fine),
#     values_from = per,
#     values_fill = 0
#   )
# 
# colnames(IGHV_fine)[2:ncol(IGHV_fine)] <-
#   paste0(colnames(IGHV_fine)[2:ncol(IGHV_fine)], "_%")
# 
# # IGHV gene per celltypeMine
# IGHV_main <- dplyr::select(gex.slim@meta.data,
#                            orig.ident,
#                            cluster_cellType_manual_main,
#                            IGHV) %>%
#   add_count(orig.ident, cluster_cellType_manual_main, IGHV, .drop = FALSE) %>%
#   distinct() %>%
#   group_by(orig.ident, cluster_cellType_manual_main) %>%
#   mutate(per =  prop.table(n)*100) %>%
#   drop_na() %>%
#   dplyr::select(orig.ident,
#                 cluster_cellType_manual_main,
#                 IGHV,
#                 per) %>%
#   pivot_wider(
#     names_from = c(IGHV, cluster_cellType_manual_main),
#     values_from = per,
#     values_fill = 0
#   )
# 
# colnames(IGHV_main)[2:ncol(IGHV_main)] <-
#   paste0(colnames(IGHV_main)[2:ncol(IGHV_main)], "_%")
 

# IGHD
IGHD <- dplyr::select(gex.slim@meta.data,
                           orig.ident,
                           IGHD) %>%
  add_count(orig.ident,
            IGHD,
            .drop = FALSE) %>%
  distinct() %>%
  group_by(orig.ident) %>%
  mutate(per =  prop.table(n)*100) %>%
  drop_na() %>%
  dplyr::select(orig.ident, IGHD, per) %>%
  pivot_wider(
    names_from = c(IGHD),
    values_from = per,
    values_fill = 0
  )

colnames(IGHD)[2:ncol(IGHD)] <-
  paste0(colnames(IGHD)[2:ncol(IGHD)], "_%")

# # IGHD gene per celltypeFine
# IGHD_fine <- dplyr::select(gex.slim@meta.data,
#                            orig.ident,
#                            cluster_cellType_manual_fine,
#                            IGHD) %>%
#   add_count(orig.ident, cluster_cellType_manual_fine, IGHD, .drop = FALSE) %>%
#   distinct() %>%
#   group_by(orig.ident, cluster_cellType_manual_fine) %>%
#   mutate(per =  prop.table(n)*100) %>%
#   drop_na() %>%
#   dplyr::select(orig.ident, cluster_cellType_manual_fine, IGHD, per) %>%
#   pivot_wider(
#     names_from = c(IGHD, cluster_cellType_manual_fine),
#     values_from = per,
#     values_fill = 0
#   )
# 
# colnames(IGHD_fine)[2:ncol(IGHD_fine)] <-
#   paste0(colnames(IGHD_fine)[2:ncol(IGHD_fine)], "_fine")
# 
# # IGHD gene per celltypeMine
# IGHD_main <- dplyr::select(gex.slim@meta.data,
#                            orig.ident,
#                            cluster_cellType_manual_main,
#                            IGHD) %>%
#   add_count(orig.ident, cluster_cellType_manual_main, IGHD, .drop = FALSE) %>%
#   distinct() %>%
#   group_by(orig.ident, cluster_cellType_manual_main) %>%
#   mutate(per =  prop.table(n)*100) %>%
#   drop_na() %>%
#   dplyr::select(orig.ident, cluster_cellType_manual_main, IGHD, per) %>%
#   pivot_wider(
#     names_from = c(IGHD, cluster_cellType_manual_main),
#     values_from = per,
#     values_fill = 0
#   )
# 
# colnames(IGHD_main)[2:ncol(IGHD_main)] <-
#   paste0(colnames(IGHD_main)[2:ncol(IGHD_main)], "_%")

# IGHJ
IGHJ <- dplyr::select(gex.slim@meta.data,
                           orig.ident,
                           IGHJ) %>%
  add_count(orig.ident,
            IGHJ,
            .drop = FALSE) %>%
  distinct() %>%
  group_by(orig.ident) %>%
  mutate(per =  prop.table(n)*100) %>%
  drop_na() %>%
  dplyr::select(orig.ident, IGHJ, per) %>%
  pivot_wider(
    names_from = c(IGHJ),
    values_from = per,
    values_fill = 0
  )

colnames(IGHJ)[2:ncol(IGHJ)] <-
  paste0(colnames(IGHJ)[2:ncol(IGHJ)], "_%")

# # IGHJ gene per celltypeFine
# IGHJ_fine <- dplyr::select(gex.slim@meta.data,
#                            orig.ident,
#                            cluster_cellType_manual_fine,
#                            IGHJ) %>%
#   add_count(orig.ident, cluster_cellType_manual_fine, IGHJ, .drop = FALSE) %>%
#   distinct() %>%
#   group_by(orig.ident, cluster_cellType_manual_fine) %>%
#   mutate(per =  prop.table(n)*100) %>%
#   drop_na() %>%
#   dplyr::select(orig.ident, cluster_cellType_manual_fine, IGHJ, per) %>%
#   pivot_wider(
#     names_from = c(IGHJ, cluster_cellType_manual_fine),
#     values_from = per,
#     values_fill = 0
#   )
# 
# colnames(IGHJ_fine)[2:ncol(IGHJ_fine)] <-
#   paste0(colnames(IGHJ_fine)[2:ncol(IGHJ_fine)], "_%")
# 
# 
# # IGHJ gene per celltypeMain
# IGHJ_main <- dplyr::select(gex.slim@meta.data,
#                            orig.ident,
#                            cluster_cellType_manual_main,
#                            IGHJ) %>%
#   add_count(orig.ident, cluster_cellType_manual_main, IGHJ, .drop = FALSE) %>%
#   distinct() %>%
#   group_by(orig.ident, cluster_cellType_manual_main) %>%
#   mutate(per =  prop.table(n)*100) %>%
#   drop_na() %>%
#   dplyr::select(orig.ident, cluster_cellType_manual_main, IGHJ, per) %>%
#   pivot_wider(
#     names_from = c(IGHJ, cluster_cellType_manual_main),
#     values_from = per,
#     values_fill = 0
#   )
# 
# colnames(IGHJ_main)[2:ncol(IGHJ_main)] <-
#   paste0(colnames(IGHJ_main)[2:ncol(IGHJ_main)], "_main")

# IGHC
# c_gene_10x <- dplyr::select(gex.slim@meta.data,
#                            orig.ident,
#                            c_gene_10x) %>%
#   add_count(orig.ident,
#             c_gene_10x,
#             .drop = FALSE) %>%
#   distinct() %>%
#   group_by(orig.ident) %>%
#   mutate(per =  prop.table(n)*100) %>%
#   drop_na() %>%
#   dplyr::select(orig.ident, c_gene_10x, per) %>%
#   pivot_wider(
#     names_from = c(c_gene_10x),
#     values_from = per,
#     values_fill = 0
#   )
# 
# colnames(c_gene_10x)[2:ncol(c_gene_10x)] <-
#   paste0(colnames(c_gene_10x)[2:ncol(c_gene_10x)], "_%")

# IGHC gene per celltypeFine
c_gene_10x_fine <- dplyr::select(gex.slim@meta.data,
                                 orig.ident,
                                 cluster_cellType_manual_fine,
                                 c_gene_10x) %>%
  add_count(orig.ident,
            cluster_cellType_manual_fine,
            c_gene_10x, .drop = FALSE) %>%
  distinct() %>%
  group_by(orig.ident, cluster_cellType_manual_fine) %>%
  mutate(per =  prop.table(n)*100) %>%
  drop_na() %>%
  dplyr::select(orig.ident, cluster_cellType_manual_fine, c_gene_10x, per) %>%
  pivot_wider(
    names_from = c(c_gene_10x, cluster_cellType_manual_fine),
    values_from = per,
    values_fill = 0
  )

colnames(c_gene_10x_fine)[2:ncol(c_gene_10x_fine)] <-
  paste0(colnames(c_gene_10x_fine)[2:ncol(c_gene_10x_fine)], "_%")

# # IGHC gene per celltypeMine
c_gene_10x_main <- dplyr::select(gex.slim@meta.data,
                                 orig.ident,
                                 cluster_cellType_manual_main,
                                 c_gene_10x) %>%
  add_count(orig.ident,
            cluster_cellType_manual_main,
            c_gene_10x, .drop = FALSE) %>%
  distinct() %>%
  group_by(orig.ident, cluster_cellType_manual_main) %>%
  mutate(per =  prop.table(n)*100) %>%
  drop_na() %>%
  dplyr::select(orig.ident, cluster_cellType_manual_main, c_gene_10x, per) %>%
  pivot_wider(
    names_from = c(c_gene_10x, cluster_cellType_manual_main),
    values_from = per,
    values_fill = 0
  )

colnames(c_gene_10x_main)[2:ncol(c_gene_10x_main)] <-
  paste0(colnames(c_gene_10x_main)[2:ncol(c_gene_10x_main)], "_%")

# IGHVDJ_cell <-
#   inner_join(IGHV_fine, IGHV_main, by = "orig.ident") %>%
#   inner_join(IGHD_fine, by = "orig.ident") %>%
#   inner_join(IGHD_main, by = "orig.ident") %>%
#   inner_join(IGHJ_fine, by = "orig.ident") %>%
#   inner_join(IGHJ_main, by = "orig.ident") %>%
#   inner_join(c_gene_10x_fine, by = "orig.ident") %>%
#   inner_join(c_gene_10x_main, by = "orig.ident")


```


## IFN score

```{r}

# get hallmark gene sets
HM <- getGenesets("/Users/gusarv/Downloads/MSigDB_Hallmark_2020.txt")

IFN <- unique(c(HM$`Interferon Alpha Response`, 
                HM$`Interferon Gamma Response`))
#IFNA <- HM$`Interferon Alpha Response`
#IFNG <- HM$`Interferon Gamma Response`

#gex.slim[rownames(gex.slim) %in% IFN, ]
#gex.slim@assays$RNA@data[rownames(gex.slim) %in% IFN, ]

#cluster_cellType_manual_fine
ALL_expr <- AggregateExpression(gex.slim, 
                                slot = "counts",
                                group.by = c("cluster_cellType_manual_fine", "orig.ident"))

ALL_expr <- as.data.frame(ALL_expr$RNA)
IFN_expr <- ALL_expr[IFN, ]
ALL_expr <- ALL_expr[!(rownames(ALL_expr) %in% IFN), ]

IFN_fine <- as.data.frame(colSums(IFN_expr, na.rm = TRUE) / colSums(ALL_expr, na.rm = TRUE) * 100)
colnames(IFN_fine)[1] <- "IFN_score"
IFN_fine$cluster_cellType_manual_fine <- substr(rownames(IFN_fine), 1, nchar(rownames(IFN_fine)) - 5)
IFN_fine$orig.ident <- substr(rownames(IFN_fine), nchar(rownames(IFN_fine)) - 3, nchar(rownames(IFN_fine)))

IFN_fine <- pivot_wider(IFN_fine, 
                        names_from = cluster_cellType_manual_fine, 
                        values_from = IFN_score) 

colnames(IFN_fine)[2:ncol(IFN_fine)] <- paste0(colnames(IFN_fine)[2:ncol(IFN_fine)], "_IFNscore")


#cluster_cellType_manual_main
ALL_expr <- AggregateExpression(gex.slim,
                                slot = "counts",
                                group.by = c("cluster_cellType_manual_main", "orig.ident"))

ALL_expr <- as.data.frame(ALL_expr$RNA)
IFN_expr <- ALL_expr[IFN, ]
ALL_expr <- ALL_expr[!(rownames(ALL_expr) %in% IFN), ]

IFN_main <- as.data.frame(colSums(IFN_expr, na.rm = TRUE) / colSums(ALL_expr, na.rm = TRUE) * 100)
colnames(IFN_main)[1] <- "IFN_score"
IFN_main$cluster_cellType_manual_main <- substr(rownames(IFN_main), 1, nchar(rownames(IFN_main)) - 5)
IFN_main$orig.ident <- substr(rownames(IFN_main), nchar(rownames(IFN_main)) - 3, nchar(rownames(IFN_main)))

IFN_main <- pivot_wider(IFN_main,
                        names_from = cluster_cellType_manual_main,
                        values_from = IFN_score)

colnames(IFN_main)[2:ncol(IFN_main)] <- paste0(colnames(IFN_main)[2:ncol(IFN_main)], "_IFNscore")

# IFN_score <- inner_join(IFN_main, IFN_fine, by = "orig.ident")


# pc_out_sc %>% ggplot(aes(x = patient_group,
#                          y = Naive_main_IFNscore,
#                          color = patient_group)) + 
#   geom_boxplot() +
#   theme_classic()
# 
# pc_out_sc %>% ggplot(aes(x = patient_group,
#                          y = Naive_main_IFNscore,
#                          color = patient_group)) + 
#   geom_boxplot() +
#   theme_classic()
# 
# pc_out_sc %>% ggplot(aes(x = patient_group,
#                          y = Transitional_main_IFNscore,
#                          color = patient_group)) + 
#   geom_boxplot() +
#   theme_classic()
# 
# pc_out_sc %>% ggplot(aes(x = patient_group,
#                          y = DN_main_IFNscore,
#                          color = patient_group)) + 
#   geom_boxplot() +
#   theme_classic()
# 
# pc_out_sc %>% ggplot(aes(x = patient_group,
#                          y = Memory_main_IFNscore,
#                          color = patient_group)) + 
#   geom_boxplot() +
#   theme_classic()


```


## mutated VDJ bp per celltype 

```{r}

mu_fine <- gex.slim@meta.data %>% 
  select(orig.ident, cluster_cellType_manual_fine, mu_freq_total, c_gene_10x) %>%
  drop_na() %>%
  group_by(orig.ident, cluster_cellType_manual_fine) %>% 
  summarise(mu_mean_fine = mean(mu_freq_total) * 100) %>% 
  pivot_wider(names_from = cluster_cellType_manual_fine, 
              values_from = mu_mean_fine,
              values_fill = 0)

colnames(mu_fine)[2:ncol(mu_fine)] <- paste0(colnames(mu_fine)[2:ncol(mu_fine)], "_VDJmutation")


mu_main <- gex.slim@meta.data %>%
  select(orig.ident, cluster_cellType_manual_main, mu_freq_total, c_gene_10x) %>%
  drop_na() %>%
  group_by(orig.ident, cluster_cellType_manual_main) %>%
  summarise(mu_mean_main = mean(mu_freq_total * 100)) %>%
  pivot_wider(names_from = cluster_cellType_manual_main,
              values_from = mu_mean_main,
              values_fill = 0)

colnames(mu_main)[2:ncol(mu_main)] <- paste0(colnames(mu_main)[2:ncol(mu_main)], "_VDJmutation")
 
# mu_freq <- inner_join(mu_main, mu_fine, by = "orig.ident")


```


## clonotypes % 

```{r}

# clonotypes for all cells per patients 
# clono_all <- gex.slim@meta.data %>% 
#   select(orig.ident, cloneType_airr_short, c_gene_10x) %>%
#   drop_na() %>%
#   select(orig.ident, cloneType_airr_short)  %>%
#   add_count(orig.ident, 
#             cloneType_airr_short,
#             .drop = FALSE) %>%
#   distinct() %>%
#   group_by(orig.ident) %>%
#   mutate(per =  prop.table(n)*100) %>%
#   dplyr::select(orig.ident,
#                 cloneType_airr_short,
#                 per) %>%
#   pivot_wider(names_from = c(cloneType_airr_short), 
#               values_from = per,
#               values_fill = 0)
# 
# colnames(clono_all)[2:ncol(clono_all)] <- paste0(colnames(clono_all)[2:ncol(clono_all)], "_clonotypes_%")


# clonotypes for cell type fine
clono_fine <- gex.slim@meta.data %>% 
  select(orig.ident, cluster_cellType_manual_fine, cloneType_airr_short, c_gene_10x) %>%
  drop_na() %>%
  select(orig.ident, cluster_cellType_manual_fine, cloneType_airr_short)  %>%
  add_count(orig.ident, 
            cluster_cellType_manual_fine,
            cloneType_airr_short,
            .drop = FALSE) %>%
  distinct() %>%
  group_by(orig.ident, cluster_cellType_manual_fine) %>%
  mutate(per =  prop.table(n)*100) %>%
  dplyr::select(orig.ident, 
                cluster_cellType_manual_fine,
                cloneType_airr_short,
                per) %>%
  pivot_wider(names_from = c(cluster_cellType_manual_fine, cloneType_airr_short), 
              values_from = per,
              values_fill = 0)
  
colnames(clono_fine)[2:ncol(clono_fine)] <- paste0(colnames(clono_fine)[2:ncol(clono_fine)], "_clonotypes_%")


# clonotypes for cell type main
clono_main <- gex.slim@meta.data %>%
  select(orig.ident, cluster_cellType_manual_main, cloneType_airr_short, c_gene_10x) %>%
  drop_na() %>%
  select(orig.ident, cluster_cellType_manual_main, cloneType_airr_short)  %>%
  add_count(orig.ident,
            cluster_cellType_manual_main,
            cloneType_airr_short,
            .drop = FALSE) %>%
  distinct() %>%
  group_by(orig.ident, cluster_cellType_manual_main) %>%
  mutate(per =  prop.table(n)*100) %>%
  dplyr::select(orig.ident,
                cluster_cellType_manual_main,
                cloneType_airr_short,
                per) %>%
  pivot_wider(names_from = c(cluster_cellType_manual_main, cloneType_airr_short),
              values_from = per,
              values_fill = 0)

colnames(clono_main)[2:ncol(clono_main)] <- paste0(colnames(clono_main)[2:ncol(clono_main)], "_clonotypes_%")


# clono <- inner_join(clono_main, clono_fine, by = "orig.ident")


```


## concatenate vars object

```{r}

# var_df <- 
#   full_join(cells_fine, IGHV, by = "orig.ident") %>%
#   full_join(IGHD, by = "orig.ident") %>%
#   full_join(IGHJ, by = "orig.ident") %>%
#   full_join(c_gene_10x_fine, by = "orig.ident") %>%
#   full_join(IFN_fine, by = "orig.ident") %>%
#   full_join(mu_fine, by = "orig.ident") %>%
#   full_join(clono_fine, by = "orig.ident")
# 
# var_df <- var_df[-28, ] #remove P024
# var_df[is.na(var_df)] <- 0


var_df <- 
  full_join(cells_main, IGHV, by = "orig.ident") %>%
  full_join(IGHD, by = "orig.ident") %>%
  full_join(IGHJ, by = "orig.ident") %>%
  full_join(c_gene_10x_main, by = "orig.ident") %>%
  full_join(IFN_main, by = "orig.ident") %>%
  full_join(mu_main, by = "orig.ident") %>%
  full_join(clono_main, by = "orig.ident")

var_df <- var_df[-28, ] #remove P024
var_df[is.na(var_df)] <- 0

#saveRDS(var_df, "../results/variables_per_patient.rds")
#var_df <- readRDS("../results/variables_per_patient.rds")


colors_df <- data.frame(names = colnames(var_df),
                        category = NA,
                        color = NA)
rownames(colors_df) <- colors_df$names

colors_df[colors_df$names %in% colnames(cells_fine)[2:ncol(cells_fine)], "category"] <- "cellType"
colors_df[colors_df$names %in% c(colnames(IGHV)[2:ncol(IGHV)],
                                 colnames(IGHD)[2:ncol(IGHD)],
                                 colnames(IGHJ)[2:ncol(IGHJ)],
                                 colnames(c_gene_10x_fine)[2:ncol(c_gene_10x_fine)]), "category"] <- "VDJ_gene"
colors_df[colors_df$names %in% colnames(c_gene_10x_fine)[2:ncol(c_gene_10x_fine)], "category"] <- "VDJ_c_gene"
colors_df[colors_df$names %in% colnames(IFN_fine)[2:ncol(IFN_fine)], "category"] <- "IFN_signature"
colors_df[colors_df$names %in% colnames(mu_fine)[2:ncol(mu_fine)], "category"] <- "VDJ_mutation"
colors_df[colors_df$names %in% colnames(clono_fine)[2:ncol(clono_fine)], "category"] <- "ClonoType"


colors_df[colors_df$names %in% colnames(cells_main)[2:ncol(cells_main)], "category"] <- "cellType"
colors_df[colors_df$names %in% c(colnames(IGHV)[2:ncol(IGHV)],
                                 colnames(IGHD)[2:ncol(IGHD)],
                                 colnames(IGHJ)[2:ncol(IGHJ)],
                                 colnames(c_gene_10x_main)[2:ncol(c_gene_10x_main)]), "category"] <- "VDJ_gene"
colors_df[colors_df$names %in% colnames(c_gene_10x_main)[2:ncol(c_gene_10x_main)], "category"] <- "VDJ_c_gene"
colors_df[colors_df$names %in% colnames(IFN_main)[2:ncol(IFN_main)], "category"] <- "IFN_signature"
colors_df[colors_df$names %in% colnames(mu_main)[2:ncol(mu_main)], "category"] <- "VDJ_mutation"
colors_df[colors_df$names %in% colnames(clono_main)[2:ncol(clono_main)], "category"] <- "ClonoType"

colors_df$category <- factor(colors_df$category, levels = unique(colors_df$category)[!is.na(unique(colors_df$category))])


```


## PCA (nipals)

```{r}

#selecting variables with significant differences between patient groups
m <- apply(var_df[, 2:ncol(var_df)], 2, function(x){glm(x ~pheno$patient_group)} ) 
m <- lapply(m, summary)
m <- t(sapply(m, function(x){x$coefficients[2:4, 4]}))

sell1 <- rowMin(m)
sell <- rownames(m)[sell1 < 0.05]

#pc_df <- full_join(pheno[, c(1:2, 8:(ncol(pheno)-1))], var_df[, c("orig.ident", sell)])

pc_df <- as.data.frame(var_df[, c("orig.ident", sell)])

rownames(pc_df) <- pc_df$orig.ident

# nipals package
pc_out <- nipals::nipals(pc_df[, sell], 
                       ncomp = 4, 
                       center = TRUE, 
                       scale = TRUE, 
                       fitted = FALSE, 
                       maxiter = 500, 
                       tol = 1e-9, 
                       gramschmidt = FALSE)

#saveRDS(pc_out, "../results/pc_out_var_df.rds")


```


## plot PCA output 

```{r}

pc_out_sc <- as.data.frame(pc_out$scores)
pc_out_sc$orig.ident <- rownames(pc_out_sc)
pc_out_sc$patient_group <- pheno$patient_group
str(pc_out_sc)

pc_out_ld <- as.data.frame(pc_out$loadings)
pc_out_ld$variable <- rownames(pc_out_ld)
pc_out_ld$category <- colors_df$category[match(pc_out_ld$var, colors_df$names)]
str(pc_out_ld)

#plot scores    
  p1.1 <- ggplot(pc_out_sc) + 
    geom_text_repel(size = 2.5,
                     aes(x = PC1, 
                         y = PC2, 
                         label = orig.ident, 
                         color = patient_group)) +
    xlab(paste0("PC1", " (", round(pc_out$eig[1], digits = 1), "%)")) +
    ylab(paste0("PC2", " (", round(pc_out$eig[2], digits = 1), "%)")) +
    ggtitle("Scores samples") + 
    geom_point(aes(x = PC1, y = PC2, color = patient_group)) +
    theme_classic() +
    geom_hline(yintercept = 0, linetype = 2, alpha = 0.4, size = 0.5) + 
    geom_vline(xintercept = 0, linetype = 2, alpha = 0.4, size = 0.5) +
    theme(legend.position = "bottom",
          legend.title=element_blank())
  
  p1.2 <- ggplot(pc_out_sc) + 
    geom_text_repel(size = 2.5,
                     aes(x = PC3, 
                         y = PC4, 
                         label = orig.ident, 
                         color = patient_group)) +
    xlab(paste0("PC3", " (", round(pc_out$eig[3], digits = 1), "%)")) +
    ylab(paste0("PC4", " (", round(pc_out$eig[4], digits = 1), "%)")) +
    ggtitle("Patient group") +
    geom_point(aes(x = PC3, y = PC4, color = patient_group)) +
    geom_hline(yintercept = 0, linetype = 2, alpha = 0.4, size = 0.5) + 
    geom_vline(xintercept = 0, linetype = 2, alpha = 0.4, size = 0.5) +
    theme_classic() +
    theme(legend.position = "bottom",
          legend.title=element_blank())

  #number of variables to display in barplot
  nvar <- 12
  
  #plot loadings PC1
  p2.1 <- pc_out_ld %>% 
    arrange(desc(PC1)) %>% 
    slice(c(1:nvar, (nrow(pc_out_ld) - nvar + 1):nrow(pc_out_ld))) %>% 
    arrange(desc(PC1)) %>%
  ggplot(aes(x = PC1, 
             y = reorder(variable, + PC1),
             fill = category)) +
    ylab("") +
    geom_bar(stat = "identity") +
    theme_classic() +
    theme(legend.position = "none") +
    scale_fill_brewer(palette = "Dark2")
  
  
  #plot loadings PC2
  p2.2 <- pc_out_ld %>% 
    arrange(desc(PC2)) %>% 
    slice(c(1:nvar, (nrow(pc_out_ld) - nvar + 1):nrow(pc_out_ld))) %>% 
    arrange(desc(PC2)) %>%
  ggplot(aes(x = PC2, 
             y = reorder(variable, + PC2),
             fill = category)) +
    ylab("") +
    geom_bar(stat = "identity") +
    theme_classic() +
    theme(legend.position = "none") +
    scale_fill_brewer(palette = "Dark2")
  
  #scatterplot loadings
  tmp1 <- pc_out_ld %>% 
    arrange(desc(PC1)) %>% 
    slice(c(1:nvar, (nrow(pc_out_ld) - nvar + 1):nrow(pc_out_ld)))
  
  tmp2 <- pc_out_ld %>% 
    arrange(desc(PC1)) %>% 
    slice(c(1:nvar, (nrow(pc_out_ld) - nvar + 1):nrow(pc_out_ld)))
  
  tmp3 <- full_join(tmp1, tmp2, by = "variable", copy = FALSE, suffix = c("", ""))
  
  p3.1 <- ggplot(tmp3) + 
    geom_text_repel(size = 2.5,
                     aes(x = PC1, 
                         y = PC2, 
                         label = variable, 
                         color = category)) +
    # xlab(paste0("PC1", " (", round(pc_out$eig[1], digits = 1), "%)")) +
    # ylab(paste0("PC2", " (", round(pc_out$eig[2], digits = 1), "%)")) +
    xlab(paste0("PC1")) +
    ylab(paste0("PC2")) +
    ggtitle("Variable Loadings") +
    geom_point(aes(x = PC1, y = PC2, color = category)) +
    theme_classic() +
    scale_color_brewer(palette = "Dark2", name = "") +
    geom_hline(yintercept = 0, linetype = 2, alpha = 0.4, size = 0.5) + 
    geom_vline(xintercept = 0, linetype = 2, alpha = 0.4, size = 0.5) +
    theme(legend.position = "bottom",
          )


```


## correlation with pheno data

```{r}

# merge PCA output with input and phenmo data
pc_out_full <- full_join(pc_out_sc, pheno)%>%
  full_join(pc_df)

# test for correlations
p_pheno <- data.frame(row.names = colnames(pc_out_full[, 7:(ncol(pc_out_full))]))
for (i in c("PC1", "PC2")){
m <- apply(pc_out_full[, rownames(p_pheno)], 2, function(x){glm(pc_out_sc[, i] ~ x)} ) 
m <- lapply(m, summary)
p_pheno[, i] <- c(t(sapply(m, function(x){x$coefficients[2, 4]})))
}

p_pheno$group <- as.factor(c(rep("age", times = 2),
                   rep("auto-Ab", times = 7),
                   rep("other", times = 12),
                   as.character(na.omit(colors_df$category[match(rownames(p_pheno), rownames(colors_df))]))))
p_pheno <- p_pheno[!(row.names(p_pheno) %in% c("SSA_Ro52", "SSA_Ro60", "SSA_and_SSB")),]


p5.1 <- ggplot(p_pheno[1:18, ], aes(x = -log10(PC1),
                                    y = -log10(PC2),
                                    color = group)) +
  geom_point() +
  geom_text_repel(size = 2,
                  aes(label = rownames(p_pheno[1:18, ]))) +
  geom_hline(yintercept = -log10(0.05), linetype = 2, alpha = 0.4, size = 0.5) + 
  geom_vline(xintercept = -log10(0.05), linetype = 2, alpha = 0.4, size = 0.5) +
  ylab(expression(paste("-log10(", italic("P"), ") PC2"))) +
  xlab(expression(paste("-log10(", italic("P"), ") PC1"))) +
  ggtitle("glm(PC1 ~ variable)") +
  ylim(0, 3) +
  xlim(0, 6.5) +
  theme_classic() +
  theme(legend.position = "bottom",
        legend.title=element_blank())


```


## plot Fig6A-C

```{r}

ggsave2(paste0("../results/Fig6_A-D_8.png"),
          plot_grid(p1.1, p3.1, p5.1, 
                    nrow = 1, 
                    ncol = 3, 
                    labels = NULL, 
                    align = "h", 
                    rel_widths = c(1, 1, 1)) + 
          bgcolor("white") + 
          border("white"),
          width = 42, height = 12, unit = "cm")


```


## plot Fig6D-E and SFig16-19 PCA 

```{r}

#pheno_var <- colnames(pheno[, 2:(ncol(pheno)-1)])
#pheno_var <- rownames(p_pheno)[p_pheno$PC1 <= 0.05]
pheno_var <- colnames(pc_out_full)[c(7:9, 12:13, 15:ncol(pc_out_full))]

# PC1 plots to list
p_list_PC1 <- list()
for (i in pheno_var){

  print(i)
  
  tmp <- select(pc_out_full,
                i, 
                PC1) %>% drop_na()
  
  # plot binary variables as boxplots and continous as scatter plots
  ifelse(max(tmp[, 1]) == 1,
  p_list_PC1[[i]] <- ggplot(tmp, 
                            aes_string(x = as.factor(tmp[, i]), 
                                       y = "PC1")) + 
    geom_boxplot(outlier.shape = NA, na.rm = TRUE) +
    ggtitle(i) +
    xlab("") +
    ylim(-0.35, 0.45) +
    #annotate("text", y = 0.32, x = 2, label = paste0("p = ", signif(p_pheno[i, 1], digits = 2))) +
    stat_compare_means(method = "t.test", label = "p.format", vjust = 0.5, na.rm = TRUE) +
    scale_x_discrete(labels = c("0" = "No", 
                                "1" = "Yes")) +
    theme_classic(), 
  p_list_PC1[[i]] <- ggplot(tmp, 
                            aes_string(x = tmp[, i], 
                                       y = "PC1")) + 
    geom_point(na.rm = TRUE) +
    ggtitle(i) +
    xlab("") +
    ylim(-0.35, 0.45) +
    geom_smooth(method = lm, se = FALSE, color = "black" , weight = 0.5) +
    stat_cor(method = "pearson", na.rm = TRUE) +
    scale_x_continuous(breaks = seq(signif(min(tmp[, 1]), digits = 1), 
                                    signif(max(tmp[, 1]), digits = 1), 
                                    by =  signif(max(tmp[, 1])/6, digits = 1))) +
    theme_classic())
  
}


# plot PC2
p_list_PC2 <- list()
for (i in pheno_var){

  print(i)
  
  tmp <- select(pc_out_full,
                i, 
                PC2) %>% drop_na()
  
  # plot binary variables as boxplots and continous as scatter plots
  ifelse(max(tmp[, 1]) == 1,
  p_list_PC2[[i]] <- ggplot(tmp, 
                            aes_string(x = as.factor(tmp[, i]), 
                                       y = "PC2")) + 
    geom_boxplot(outlier.shape = NA, na.rm = TRUE) +
    ggtitle(i) +
    xlab("") +
    ylim(-0.35, 0.45) +
    #annotate("text", y = 0.32, x = 2, label = paste0("p = ", signif(p_pheno[i, 2], digits = 2))) +
    stat_compare_means(method = "t.test", label = "p.format", vjust = 0.5, na.rm = TRUE) +
    scale_x_discrete(labels = c("0" = "No", 
                                "1" = "Yes")) +
    theme_classic(),
  p_list_PC2[[i]] <- ggplot(tmp, 
                            aes_string(x = tmp[, i], 
                                       y = "PC2")) + 
    geom_point(na.rm = TRUE) +
    ggtitle(i) +
    xlab("") +
    ylim(-0.35, 0.45) +
    geom_smooth(method = lm, se = FALSE, color = "black", weight = 0.7) +
    stat_cor(method = "pearson", na.rm = TRUE) +
    scale_x_continuous(breaks = seq(signif(min(tmp[, 1]), digits = 1), 
                                    signif(max(tmp[, 1]), digits = 1), 
                                    by = signif(max(tmp[, 1])/6, digits = 1))) +
    theme_classic())
  
}

# plot all patient variables for PC1 and PC2
ggsave2(paste0("../results/SFig16_PC1.png"),
          plot_grid(plotlist = p_list_PC1[pheno_var[1:18]], 
                    nrow = 6, 
                    ncol = 3, 
                    labels = NULL, 
                    align = "hv") + 
          bgcolor("white") + 
          border("white"),
          width = 20, height = 30, unit = "cm")

ggsave2(paste0("../results/SFig17_PC2.png"),
          plot_grid(plotlist = p_list_PC2[pheno_var[1:18]], 
                    nrow = 6, 
                    ncol = 3, 
                    labels = NULL, 
                    align = "hv") + 
          bgcolor("white") + 
          border("white"),
          width = 20, height = 30, unit = "cm")


# plot GEX and VDJ variables for PC1 and PC2
#length(pheno_var[19:length(pheno_var)]) #[1] 33
ggsave2(paste0("../results/SFig18_PC1.png"),
          plot_grid(plotlist = p_list_PC1[pheno_var[19:length(pheno_var)]], 
                    nrow = 7, 
                    ncol = 5, 
                    labels = NULL, 
                    align = "hv") + 
          bgcolor("white") + 
          border("white"),
          width = 30, height = 30, unit = "cm")

ggsave2(paste0("../results/SFig19_PC2.png"),
          plot_grid(plotlist = p_list_PC2[pheno_var[19:length(pheno_var)]], 
                    nrow = 7, 
                    ncol = 5, 
                    labels = NULL, 
                    align = "hv") + 
          bgcolor("white") + 
          border("white"),
          width = 30, height = 30, unit = "cm")


# plot select patient variables for PC1 
p_pat_var <- c("SSA", "SSB", "ANA", "RF", 
  "Age_symptom_onset", "Age_diagnosis",
  "Focus_score_Greenspan", "P-IgG", "Leucopenia")

ggsave2(paste0("../results/Fig6D.png"),
          plot_grid(plotlist = p_list_PC1[p_pat_var], 
                    nrow = 3, 
                    ncol = 3, 
                    labels = NULL, 
                    align = "hv") + 
          bgcolor("white") + 
          border("white"),
          width = 20, height = 20, unit = "cm")


# plot select GEX and VDJ variables for PC1 
p_gex_var <- c("Memory_%", "DN_%", "Naive_%", "Transitional_%",
  "Memory_IFNscore", "DN_IFNscore", "Naive_IFNscore", "Transitional_IFNscore", 
  "Memory_VDJmutation", "Transitional_VDJmutation",
  "IGHV1-69_%", "IGHV3-66_%")

ggsave2(paste0("../results/Fig6E.png"),
          plot_grid(plotlist = p_list_PC1[p_gex_var], 
                    nrow = 3, 
                    ncol = 4, 
                    labels = NULL, 
                    align = "hv") + 
          bgcolor("white") + 
          border("white"),
          width = 28, height = 18, unit = "cm")


```





## Save rds GEX_xxx

```{r}

#saveRDS(gex, file = "../results/GEX_xxx.rds")

```


## Print sessionInfo()

```{r}

sessionInfo()

```



