...
saveRDS(cells_main, file = "../results/cells_main.rds")
...
saveRDS(IFN_main, file = "../results/IFN_main.rds")
...
pc_out <- readRDS("../results/pc_out_var_df.rds")
...
p5.1 <- ggplot(p_pheno[1:17, ], aes(x = -log10(PC1),
                                    y = -log10(PC2),
                                    color = group)) +
  geom_point() +
  geom_text_repel(size = 2,
                  aes(label = rownames(p_pheno[1:17, ]))) +
...
p1 <- plot_grid(plotlist = p_list_PC1[pheno_var[1:18]], 
                    nrow = 6, 
                    ncol = 3, 
                    labels = NULL, 
                    align = "hv") + 
...
ggsave2(paste0("../results/SFig18.png"),
...
ggsave2(paste0("../results/SFig18.pdf"),
          plot_grid(p1, p_legend, 
                    ncol = 1, 
                    labels = NULL, 
                    align = "hv",
...
p1 <- plot_grid(plotlist = p_list_PC2[pheno_var[1:18]], 
                    nrow = 6, 
                    ncol = 3, 
                    labels = NULL, 
                    align = "hv") + 
...
p1 <- plot_grid(plotlist = p_list_PC1[pheno_var[19:length(pheno_var)]], 
                    nrow = 7, 
                    ncol = 5, 
                    labels = NULL, 
                    align = "hv") + 
...
p1 <- plot_grid(plotlist = p_list_PC2[pheno_var[19:length(pheno_var)]], 
                    nrow = 7, 
                    ncol = 5, 
                    labels = NULL, 
                    align = "hv") + 
...
ggsave2(paste0("../results/Fig5D-E.png"),
          ...
          bgcolor("white") + 
          border("white"),
          width = 42, height = 18, unit = "cm", dpi = 200)
...
#CD21 "CR2"
p1 <- FeaturePlot(gex, "CR2", reduction = "umap_2", split.by = "patient_group", order = TRUE)
p1 <- VlnPlot(gex, "CR2", split.by = "patient_group", group.by = "cluster_cellType_manual_main")
p1 <- DotPlot(gex, features = "CR2", group.by = "pat_gr_cell_type")
group.by = "cluster_cellType_manual_main"

DoHeatmap(subset(gex, downsample = 100), features = "CR2", size = 3, slot = "data")

#p1 <- RidgePlot(gex, features = "CR2", group.by = "pat_gr_cell_type")

gex$pat_gr_cell_type <- paste(gex$patient_group, gex$cluster_cellType_manual_main, sep = "_")
