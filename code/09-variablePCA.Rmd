...
#make data columns numerical  binary
pheno[pheno == "NO"] <- 0
pheno[pheno == "YES"] <- 1
pheno[pheno == "MD"] <- NA
...
ggsave2(paste0("../results/Fig6_A-C_5.png"),
          plot_grid(p1.1, p3, 
                    nrow = 1, 
                    ncol = 2, 
                    labels = NULL, 
...
pc_out
pc_out_sc
head(var_df[, c("orig.ident", sell)])
head(pheno)

p_pheno <- data.frame(row.names = colnames(pheno[, 2:(ncol(pheno)-1)]))

for (i in c("PC1", "PC2")){
m <- apply(pheno[, 2:(ncol(pheno)-1)], 2, function(x){glm(pc_out_sc[, i] ~ x)} ) 
m <- lapply(m, summary)
p_pheno[, i] <- c(t(sapply(m, function(x){x$coefficients[2, 4]})))
}

p4 <- ggplot(p_pheno, aes(x = -log10(PC1),
                    y = -log10(PC2))) +
  geom_point() +
  geom_text_repel(size = 2.5,
                  aes(label = rownames(p_pheno))) +
...
p_var <- data.frame(row.names = colnames(var_df[, sell]))

for (i in c("PC1", "PC2")){
m <- apply(var_df[, sell], 2, function(x){glm(pc_out_sc[, i] ~ x)} ) 
m <- lapply(m, summary)
p_var[, i] <- c(t(sapply(m, function(x){x$coefficients[2, 4]})))
}

p5 <- ggplot(p_var, aes(x = -log10(PC1),
                  y = -log10(PC2))) +
  geom_point() +
  geom_text_repel(size = 2.5,
                     aes(label = rownames(p_var))) +
  geom_hline(yintercept = -log10(0.05), linetype = 2, alpha = 0.4, size = 0.5) + 
  geom_vline(xintercept = -log10(0.05), linetype = 2, alpha = 0.4, size = 0.5) +
  theme_classic()


ggsave2(paste0("../results/Fig6_A-D.png"),
          plot_grid(p1.1, p2, p3, p4, 
                    nrow = 1, 
                    ncol = 4, 
                    labels = NULL, 
                    align = "h", 
...
for (i in colnames(pc_df)[2:ncol(pc_df)]){
...
ggsave2(paste0("../results/SFig15_PC1_1.png"),
...
ggsave2(paste0("../results/SFig15_PC2_1.png"),
...
# plot PC1 and PC2 for pheno variables
ggsave2(paste0("../results/SFig16_PC1.png"),
          plot_grid(plotlist = p_list_PC1[pheno_var], 
                    nrow = 5, 
                    ncol = 4, 
                    labels = NULL, 
                    align = "hv") + 
...
ggsave2(paste0("../results/SFig16_PC2.png"),
          plot_grid(plotlist = p_list_PC2[pheno_var], 
                    nrow = 5, 
                    ncol = 4, 
                    labels = NULL, 
                    align = "hv") + 
